...
function cls($s){ $s = trim($s); return $s!==''? $s : 'Clase'; }
function camel($s){ return lcfirst(preg_replace('/[_\s]+(.)/e','strtoupper("$1")', strtolower($s))); }
...
function tipo_sql($t){
  ...
  if ($t==='decimal' || preg_match('~decimal\(\d+,\d+\)~',$t)) return $t==='decimal' ? 'decimal(10,2)' : $t;
...
foreach(($doc['nodos']??[]) as $n){
  ...
  foreach($atr_lines as $linea){
    ...
    if ($s==='') continue;
    $vis = '';
    if ($s[0]=='+'||$s[0]=='#'||$s[0]=='-') { $vis=$s[0]; $s=trim(substr($s,1)); }
    $part = explode(':',$s,2);
    ...
    if ($campo==='id'){ $pk=true; $tieneId=true; if ($sql==='integer') $sql='bigint'; }
  ...
  $clases[] = $clase;
  $por_id[$id_actual] = $clase;      // (se puede dejar, ya no se usa para modificar)
  $id2idx[$id_actual] = count($clases)-1;  // <--- guardamos el índice real
}

// helper para añadir FK evitando duplicados
$add_fk = function(array &$tabla, array $ref) {
  $fk_name = 'id_'.strtolower($ref['name']);
  foreach($tabla['attrs'] as $x){ if($x['campo']===$fk_name) return; } // ya existe
  $tabla['attrs'][] = [
...
foreach(($doc['aristas']??[]) as $a){
  ...
  if (esUno($co) && esMuchos($cd)){
    $add_fk($clases[$idx_d], $clases[$idx_o]);  // <--- modificamos por referencia
  }
  // A(*) ---- (1)B  => A lleva FK a B
  if (esMuchos($co) && esUno($cd)){
    $add_fk($clases[$idx_o], $clases[$idx_d]);  // <--- modificamos por referencia
  }
}
...
function flutter_main_dart($clases){
  $items = [];
  foreach($clases as $c){ $items[] = "MenuItem('${$c['name']}', '/${strtolower($c['name'])}', '${$c['name']}')"; }
  $items_txt = implode(",\n        ", $items);
  return <<<DART
      ...
      body: ListView.builder(
        ...
        itemBuilder: (_,i){
          ...
          return ListTile(
            ...
            onTap: ()=> Navigator.of(ctx).push(MaterialPageRoute(builder: (_)=> CrudPage(entidad: it.entidad))),
          );
        }),
    );
  }
...
class _CrudPageState extends State<CrudPage>{
  ...
  Future<void> _load() async{
    final l = await Api.listar(widget.entidad);
    setState(()=> data = l.cast<Map<String,dynamic>>());
  }
  ...
  @override Widget build(BuildContext ctx){
    return Scaffold(
      appBar: AppBar(title: Text(widget.entidad)),
      body: Column(
        children: [
          Padding(
            padding: const EdgeInsets.all(8),
            child: Row(children: [
              Expanded(child: TextField(decoration: const InputDecoration(labelText:'campo'), onChanged:(v)=>form['campo']=v)),
              const SizedBox(width:8),
              FilledButton(onPressed:_crear, child: const Text('Agregar')),
            ]),
          ),
...
foreach($clases as $c){
  ...
  foreach($c['attrs'] as $a){
    $col = "\"{$a['campo']}\" ". ($a['pk'] && $a['ai'] ? 'bigserial' : $a['sql']) . ($a['nullable']?'':' NOT NULL');
...
if ($zip->open($tmp, ZipArchive::CREATE|ZipArchive::OVERWRITE)!==TRUE){
  http_response_code(500); echo "No se pudo crear ZIP"; exit;
}
